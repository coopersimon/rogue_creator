...
named!(p_return<&[u8], Box<Statement> >,
    do_parse!(
        tag!(RETURN)        >>
        e: opt!(do_parse!(
            multispace  >>
...
fn make_id(start: &[u8], end: Option<&[u8]>) -> String {
    let out = str::from_utf8(start).unwrap();
    ...
    match s.as_str() {
        FUNC    => panic!("Id matches keyword"),
        VAR     => panic!("Id matches keyword"),
        RETURN  => panic!("Id matches keyword"),
        FOR     => panic!("Id matches keyword"),
        WHILE   => panic!("Id matches keyword"),
        IF      => panic!("Id matches keyword"),
        ELSE    => panic!("Id matches keyword"),
        TRUE    => panic!("Id matches keyword"),
        FALSE   => panic!("Id matches keyword"),
        _ => s,
    }
}
